...
import { errorHelper, logger, getText, vietnameseToEnglish } from '../../../../utils/index.js';
...
export default async (req, res) => {
  ...
  const params = {
    Bucket: bucketName,
    Key: vietnameseToEnglish(user.name).replace(/\s/g, '').toLowerCase() + '/' + user._id + '/' + Date(Date.now()).toLowerCase().substring(0, 15).replace(/\s/g, '-'),
